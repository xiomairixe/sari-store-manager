...
const S = {
  ...
  customerCard: (exceeded) => ({
    backgroundColor: "#fff",
    borderRadius: "16px",
    padding: "14px 16px",
    boxShadow: exceeded ? "0 0 0 1.5px #fca5a5" : "0 1px 4px rgba(0,0,0,0.06)",
  }),
  cardTop: {
    display: "flex",
    justifyContent: "space-between",
  ...
  balanceAmount: (exceeded) => ({
    fontSize: "17px",
    fontWeight: "700",
    color: exceeded ? "#ef4444" : "#1a1a2e",
    textAlign: "right",
  }),
...
export default function Utang() {
  ...
  const [modal, setModal] = useState(null); // "newCustomer" | "addUtang" | "payment"
  ...
  const handleNewCustomer = async (e) => {
    ...
    if (!form.customerName || !form.customerName.trim()) {
      return setError("Please enter a customer name.");
    }
  ...
  const handleAddUtang = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${BASE_URL}/utang/customers/${selectedCustomer.id}/add`, {
  ...
  const handlePayment = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.put(`${BASE_URL}/utang/customers/${selectedCustomer.id}/pay`, {
  ...
  const handleDeleteCustomer = async (customer) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/utang/customers/${customer.id}`);
      fetchCustomers();
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      ...
      <div style={S.page}>
        <div style={S.header}>
          <h1 style={S.title}>Customer Credit (Utang)</h1>
        </div>
        ...
        <div style={S.body}>
          ...
          {customers.length === 0 ? (
            ...
            customers.map((c) => {
              ...
              return (
                <div key={c.id} style={S.customerCard(exceeded)}>
                  <div style={S.cardTop}>
                    <div>
                      ...
                      {c.phone && (
                        <div style={S.customerPhone}>
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.81a16 16 0 0 0 6 6l.92-.92a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.73 16z" />
                          </svg>
                          {c.phone}
                        </div>
                      )}
                    ...
                    <div>
                      <div style={S.balanceAmount(exceeded)}>
                        ₱{balance.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                      </div>
                      <div style={S.limitText}>
                        Limit: ₱{limit.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={S.progressWrap}>
                    <div style={S.progressBar(pct, exceeded)} />
                  </div>
                  ...
                  <div style={S.btnRow}>
                    ...
                    <button style={S.deleteBtn} onClick={() => handleDeleteCustomer(c)} title="Delete customer">🗑</button>
                    </div>
                </div>
              );
            })
    ...
    <div style={S.modal}>
      ...
      <div style={{ marginTop: "16px", textAlign: "center" }}>
        <span style={{
          ...
          fontWeight: "700",
          backgroundColor:
            selectedCustomer.status === "paid"    ? "#dcfce7" :
            selectedCustomer.status === "partial" ? "#fef9c3" : "#fee2e2",
          color:
            selectedCustomer.status === "paid"    ? "#16a34a" :
            selectedCustomer.status === "partial" ? "#ca8a04" : "#ef4444",
        }}>
      ...
      <div style={{ marginTop: "16px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#9ca3af", marginBottom: "6px" }}>
          ...
          <span>
            {Math.min(
              Math.round((parseFloat(selectedCustomer.balance) / parseFloat(selectedCustomer.creditLimit || 1)) * 100),
              100
            )}%
          </span>
        </div>
        <div style={S.progressWrap}>
          <div style={S.progressBar(
            (parseFloat(selectedCustomer.balance) / parseFloat(selectedCustomer.creditLimit || 1)) * 100,
            parseFloat(selectedCustomer.balance) > parseFloat(selectedCustomer.creditLimit)
          )} />
        </div>
      </div>
    </div>
        ...
        {modal === "newCustomer" && (
          <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && closeModal()}>
            <div style={S.modal}>
              <div style={S.modalHeader}>
                <h2 style={S.modalTitle}>New Customer</h2>
                <button style={S.closeBtn} onClick={closeModal}>✕</button>
              </div>
              ...
              <form onSubmit={handleNewCustomer}>
                ...
                <input
                  style={S.input}
                  placeholder="e.g. Ate Lorna Cruz"
                ...
                />
                <button type="submit" style={S.submitBtn("#1a1a2e")} disabled={loading}>
        ...
        {modal === "addUtang" && selectedCustomer && (
          <div style={S.overlay} onClick={(e) => e.target === e.currentTarget && closeModal()}>
            <div style={S.modal}>
              <div style={S.modalHeader}>
                <h2 style={S.modalTitle}>Add Utang</h2>
                <button style={S.closeBtn} onClick={closeModal}>✕</button>
              </div>
              <p style={{ margin: "0 0 16px", color: "#9ca3af", fontSize: "13px" }}>
                Customer: <strong style={{ color: "#1a1a2e" }}>{selectedCustomer.customerName}</strong>
              </p>
              ...
              <form onSubmit={handleAddUtang}>
                ...
                <input
                  style={S.input}
                  placeholder="e.g. 2 cans of corned beef"
                  value={form.notes || ""}
                  onChange={(e) => setForm({ ...form, notes: e.target.value })}
                />
                <button type="submit" style={S.submitBtn("#ef4444")} disabled={loading}>
            ...
            <div style={S.modal}>
              ...
              <p style={{ margin: "0 0 16px", color: "#9ca3af", fontSize: "13px" }}>
                ...
                <strong style={{ color: "#ef4444" }}>
                  ₱{parseFloat(selectedCustomer.balance || 0).toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                </strong>
              </p>
              ...
              <form onSubmit={handlePayment}>
                ...
                <input
                  ...
                  autoFocus
                />
                <button type="submit" style={S.submitBtn("#16a34a")} disabled={loading}>
